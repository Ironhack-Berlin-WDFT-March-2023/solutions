...
function FoodList() {
  ...
  const [updatedFoods, setUpdatedFoods] = useState(foods); // Array that we render/show as a list
  ...
  const filterFoodList = (char) => {
    ...
    if (char === '') {
      ...
      filteredFoods = foods.filter((eachFood) => {
  ...
  return (
    <div className="App">
      <Button onClick={toggleForm} className="toggle-btn">{formShowing ? 'Hide Form' : 'Add New Food'}</Button>
